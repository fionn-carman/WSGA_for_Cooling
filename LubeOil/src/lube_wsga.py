...
# Structural Constraints — same as cooling GA
MAX_HEAVY_ATOMS = args.max_heavy_atoms
MIN_HEAVY_ATOMS = 5
MAX_CARBONS = args.max_heavy_atoms
MAX_OXYGENS = 6
...
def main():
    ...
    # Lubricant task: add viscosity_100C (trained for DVI), drop fom1_40C/dc
    # (dielectric constant is not a lubricant constraint; flash point optional).
    thermo_targets = [
        "bp", "mp", "fp",
    ...
    evaluated_df = add_dvi_and_lube_fitness(
        evaluated_df,
    ...
    for gen in range(1, NUM_GENERATIONS + 1):
        ...
        evaluated_offspring_df = add_dvi_and_lube_fitness(
            evaluated_offspring_df,
            weight_profile=WEIGHT_PROFILE,
        )
            ...
            if generations_since_improvement >= STAGNATION_WINDOW:
                if total_restarts < MAX_STAGNATION_RESTARTS:
                    ...
                    if len(fresh_smiles) > 0:
                        ...
                        fresh_evaluated = add_dvi_and_lube_fitness(
                            fresh_evaluated,
                            weight_profile=WEIGHT_PROFILE,
                        )
    ...
    # Save top molecules by raw FOM_LUBE (ignoring MolPrice penalty)
    raw_col = "FOM_LUBE_raw" if "FOM_LUBE_raw" in elite_df.columns else "FOM_LUBE"
    valid_elite = elite_df[elite_df["is_valid"] == 1] if "is_valid" in elite_df.columns else elite_df
    output_top_fom1_path = None
    if raw_col in valid_elite.columns and not valid_elite.empty:
        final_top_fom1_df = (
            valid_elite
    ...
    if output_top_fom1_path:
        print(f"Saved top {TOP_FINAL} by raw FOM_LUBE to: {output_top_fom1_path}")
