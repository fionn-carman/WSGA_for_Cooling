...
The SHAP analysis provides a physically grounded interpretation of the XGBoost models and reveals which molecular properties most strongly influence cooling performance. The beeswarm plots (Figures~\ref{fig:shap_beeswarm_40} and~\ref{fig:shap_beeswarm_100}) show that a small number of descriptors dominate the predictions at both temperatures, with the top 5--10 features accounting for the majority of the total SHAP magnitude. This concentrated importance structure is consistent with the physical nature of FOM1, which depends on a handful of thermophysical properties.

\paragraph{Dominant descriptors.}
At both temperatures, the Kier--Hall connectivity index \texttt{Chi0} is by far the most influential descriptor (mean $|\text{SHAP}| = 4.72$ at \SI{40}{\degreeCelsius}, $7.95$ at \SI{100}{\degreeCelsius}). \texttt{Chi0} sums the reciprocal square roots of vertex degrees across all bonds and serves as a proxy for molecular size and branching --- larger, more extended molecules have higher \texttt{Chi0} values. Its dominance reflects the strong dependence of FOM1 on bulk transport properties (viscosity, thermal conductivity) that scale with molecular size. The closely related \texttt{Chi0v}, which incorporates valence electron counts, ranks second at \SI{40}{\degreeCelsius} and fourth at \SI{100}{\degreeCelsius}, reinforcing that molecular connectivity and heteroatom content are primary drivers of cooling performance.

\texttt{FpDensityMorgan1} (Morgan fingerprint density at radius 1) is the third most important feature at both temperatures. This descriptor captures the ratio of unique structural fragments to molecular size and effectively encodes molecular complexity --- molecules with more diverse local chemical environments relative to their size tend to have different thermophysical behaviour. \texttt{SlogP\_VSA4}, which measures the van der Waals surface area contribution from atoms with intermediate lipophilicity, ranks consistently in the top four, linking cooling performance to the balance between polar and non-polar surface area --- a key factor governing liquid-phase intermolecular interactions, heat capacity, and thermal conductivity.

\texttt{BCUT2D\_LOGPHI} (the highest eigenvalue of the Burden matrix weighted by atomic log\,P contributions) is the fifth most important descriptor at \SI{40}{\degreeCelsius} but drops to tenth at \SI{100}{\degreeCelsius}. This descriptor captures the distribution of hydrophobicity across the molecular topology. Its diminished importance at higher temperatures may reflect the reduced influence of hydrophobic interactions on transport properties as thermal energy increases.

\paragraph{Temperature-dependent shifts.}
The cross-temperature comparison (Figure~\ref{fig:shap_cross_temp}) reveals both stability and shifts in descriptor importance. \texttt{Chi0} nearly doubles in importance from \SI{40}{\degreeCelsius} to \SI{100}{\degreeCelsius}, suggesting that molecular size becomes an even stronger predictor of cooling performance at elevated temperatures, likely because viscosity (which scales strongly with molecular size) becomes a more limiting factor in the FOM1 expression at higher temperatures. \texttt{MaxAbsEStateIndex}, which encodes the most extreme electrotopological state in the molecule (typically associated with highly polar or reactive functional groups), rises from tenth at \SI{40}{\degreeCelsius} to fifth at \SI{100}{\degreeCelsius}, indicating that the presence of strongly polar groups has greater influence on high-temperature cooling performance. Conversely, \texttt{Chi4n} (a higher-order connectivity index capturing four-bond path topologies) drops substantially in ranking at \SI{100}{\degreeCelsius}, suggesting that fine structural details of molecular branching become less relevant at elevated temperatures where bulk properties dominate.

\paragraph{Physical interpretation.}
Taken together, the SHAP results paint a coherent physical picture: FOM1 is primarily governed by (i)~molecular size and connectivity (\texttt{Chi0}, \texttt{Chi0v}, \texttt{Chi1n}), which control viscosity and thermal conductivity; (ii)~the distribution of polar and non-polar surface area (\texttt{SlogP\_VSA4}, \texttt{BCUT2D\_LOGPHI}, \texttt{SMR\_VSA5}), which influences intermolecular forces and heat capacity; and (iii)~atomic charge distribution (\texttt{MinAbsPartialCharge}, \texttt{MaxAbsEStateIndex}), which captures the strength of dipolar interactions. This hierarchy suggests that ideal cooling fluids are small, compact molecules with moderate polarity and low viscosity --- consistent with known engineering heuristics for selecting heat transfer fluids.

The interpretability afforded by SHAP analysis is a practical advantage of XGBoost on descriptors over black-box alternatives such as ChemBERTa embeddings, where the latent features lack direct physical meaning. These insights can directly guide molecular design: for instance, screening libraries could be pre-filtered for molecules with low \texttt{Chi0} (small size) and moderate \texttt{SlogP\_VSA4} (balanced polarity) to enrich for high-FOM1 candidates.
