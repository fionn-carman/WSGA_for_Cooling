...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/combined/shap_cross_temperature.pdf}
...
The SHAP analysis provides a physically grounded interpretation of the XGBoost models and reinforces the conclusion that descriptor-based representations are well-suited for FOM1 prediction. The beeswarm plots (Figures~\ref{fig:shap_beeswarm_40} and~\ref{fig:shap_beeswarm_100}) reveal that a relatively small number of descriptors dominate the predictions at both temperatures, with the top 5--10 features accounting for the majority of the total SHAP magnitude. This concentrated importance structure is consistent with the physical nature of FOM1, which depends on a handful of thermophysical properties.

The cross-temperature comparison (Figure~\ref{fig:shap_cross_temp}) is particularly informative: descriptors related to bulk molecular properties (e.g., molecular weight, topological indices, hydrogen bonding capacity) tend to maintain their importance across both temperatures, while descriptors capturing more temperature-sensitive properties may shift in relative ranking. This aligns with the observation that model accuracy degrades at \SI{100}{\degreeCelsius} --- the increased thermal energy introduces additional structure--property relationships that are harder to capture with room-temperature-derived descriptors.

The interpretability afforded by SHAP analysis is a practical advantage of XGBoost on descriptors over black-box alternatives such as ChemBERTa embeddings, where the latent features lack direct physical meaning. This interpretability can guide future descriptor engineering and inform physical understanding of what drives cooling performance.
...
Several directions could improve FOM1 prediction accuracy and utility:

\begin{itemize}
    \item \textbf{Graph Neural Networks (GNNs):} Architectures such as Message Passing Neural Networks (MPNNs) or Graph Attention Networks (GATs) operate directly on molecular graphs (atoms as nodes, bonds as edges) rather than requiring hand-crafted features or SMILES string processing. GNNs have shown strong performance on molecular property prediction benchmarks and could potentially capture structure--property relationships that neither descriptors nor fingerprints encode.

    \item \textbf{Expanded training data:} The current dataset is modest in size, which limits the effectiveness of data-hungry models (transformers, deep MLPs). Incorporating additional FOM1 measurements, calculated FOM1 values from molecular simulation, or related transfer-learning targets (individual thermophysical properties) could improve performance, particularly for the neural network architectures.

    \item \textbf{Multi-task learning:} Rather than predicting FOM1 at each temperature independently, a multi-task model could predict FOM1 at both temperatures simultaneously, sharing information between the two tasks. This could improve generalisation, especially at \SI{100}{\degreeCelsius} where performance is weaker.

    \item \textbf{Feature importance analysis:} SHAP (SHapley Additive exPlanations) analysis of the XGBoost models would reveal which specific molecular descriptors are most predictive of FOM1, providing chemical insight into what makes a good coolant and guiding molecular design.

    \item \textbf{Domain adaptation for extreme temperatures:} The performance degradation at \SI{100}{\degreeCelsius} suggests that high-temperature regimes involve different structure--property relationships. Domain adaptation techniques or temperature-conditioned models could bridge this gap.
\end{itemize}
