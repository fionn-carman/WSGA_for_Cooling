#!/usr/bin/env python3
"""
XGBoost baseline for FOM1 prediction.

Features: RDKit descriptors + Morgan fingerprints (concatenated ~2183 features).
Uses RandomizedSearchCV with 1000 iterations and 5-fold inner CV.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train XGBoost baseline for FOM1 prediction",
    ...
    X_desc = df[descriptor_columns].values
    X_fp = fps
    X_all = np.hstack([X_desc, X_fp])
    y = df[args.target].values
    logger.info("Target: %s", args.target)
    logger.info("Feature matrix: %s (desc=%d + fp=%d)", X_all.shape, X_desc.shape[1], X_fp.shape[1])
    ...
    for fold_id in folds:
        ...
        pred_df.to_csv(data_dir / f"xgboost_fold{fold_id}_predictions.csv", index=False)
        ...
        with open(data_dir / f"xgboost_fold{fold_id}_metrics.json", "w") as f:
            json.dump(metrics, f, indent=2)

        # Save model
        joblib.dump({"model": best_model, "scaler": scaler, "params": search.best_params_},
                     data_dir / f"xgboost_fold{fold_id}_model.joblib")
