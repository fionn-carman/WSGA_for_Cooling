#!/usr/bin/env python3
"""
MLP on RDKit descriptors for FOM1 prediction.

Architecture: Input → 256 → ReLU → Dropout(0.1) → 128 → ReLU → Dropout(0.1) → 64 → ReLU → 1
"""
...
def train_one_fold(X_train, y_train, X_test, y_test, input_dim, seed, device):
    """Train MLP on one fold, return predictions and training history."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train MLP on RDKit descriptors for FOM1",
    ...
    for fold_id in folds:
        logger.info("=" * 50)
        logger.info("FOLD %d — MLP Descriptors", fold_id)
        logger.info("=" * 50)
        t0 = time.time()
        ...
        pd.DataFrame({"SMILES": df.iloc[test_idx]["SMILES"].values, "y_true": y_test, "y_pred": y_pred})\
            .to_csv(data_dir / f"mlp_descriptors_fold{fold_id}_predictions.csv", index=False)

        with open(data_dir / f"mlp_descriptors_fold{fold_id}_metrics.json", "w") as f:
            json.dump({"model": "mlp_descriptors", "target": args.target, "fold": fold_id, "r2": float(r2),
        ...
        torch.save({"state_dict": best_state, "input_dim": X_train.shape[1]},
                    data_dir / f"mlp_descriptors_fold{fold_id}_model.pt")

        # Save training curves
        with open(data_dir / f"mlp_descriptors_fold{fold_id}_history.json", "w") as f:
            json.dump(history, f)

    logger.info("MLP Descriptors training complete.")
