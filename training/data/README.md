...
training and the full NIST WTT multi-temperature extraction.
`wtt_multitemp_merged.csv` is the raw merged master with all 26,466 molecules
(including heteroatoms — N, S, halogens, etc.) and all properties (density,
viscosity, TC, Cpsat) at 0-100C in 10C steps. The nist_8100 dataset is the
CHO-only subset after filtering and outlier removal.
The per-property `*_cleaned.csv` files are the old ~850-molecule dataset
spin-offs with saturated liquid Cp (not ideal gas).
...
| wtt_multitemp_merged.csv | Full NIST WTT extraction (all elements, all properties merged) | 26,466 | 60 |
