#!/usr/bin/env python3
"""
Publication-quality XGBoost training pipeline for old dataset (~833 molecules).

Identical methodology to nist8100/train_single_temp.py for direct comparison
of dataset size impact (old ~833 vs NIST 8100 ~8082 molecules).
...
Usage:
    python train_single_temp.py                        # all 5 properties
...
MODELS = {
    "density":   {"csv": "Density_40C_g_cm^3_cleaned.csv",                        "col_40": "Density_40C_g_cm^3",            "log": False},
...
DATA_DIR = SCRIPT_DIR.parent.parent / "data" / "old_dataset"
...
def main():
    parser = argparse.ArgumentParser(
        description="Train old-dataset XGBoost models (Mordred + Optuna)",
    ...
    # Summary table
    print(f"\n{'=' * 80}")
    print("OLD DATASET MODELS (5-fold nested CV, Mordred+RDKit, Optuna)")
    print(f"{'=' * 80}")
    ...
    with open(OUTPUT_DIR / "old_dataset_results.json", "w") as f:
