#!/usr/bin/env python3
"""
Publication-quality XGBoost training pipeline for CHNO models.

Mordred + RDKit descriptors, Optuna nested CV, SHAP analysis, diagnostic plots.
Trains both thermophysical (40C) and constraint models.
...
Usage:
    python train_single_temp.py                          # all 10 properties
    python train_single_temp.py --properties density     # single property
    python train_single_temp.py --properties fp bp mp dc # constraints only
...
UNITS = {
    ...
    "dc_cho667_nolog": "Dielectric constant",
}

MODELS = {
    # Thermophysical (40C)
    "density":   {"csv": "density_chno_cleaned.csv",   "col_40": "density_40C_g_cm3",  "log": False},
    "viscosity": {"csv": "viscosity_chno_cleaned.csv", "col_40": "kv_40C_cSt",         "log": True},
    "tc":        {"csv": "tc_chno_cleaned.csv",        "col_40": "tc_40C_W_mK",        "log": False},
    "cpsat":     {"csv": "cpsat_chno_cleaned.csv",     "col_40": "cpsat_40C_J_K_mol",  "log": False},
    "beta":      {"csv": "beta_chno_cleaned.csv",      "col_40": "beta_40C",           "log": False},
    "fom1":      {"csv": "fom1_chno_cleaned.csv",      "col_40": "fom1_40C",           "log": True},
    # Constraints
    ...
    "dc_cho667_nolog": {"csv": "constraints/DC_exp_cleaned_cho667.csv", "col_40": "DC_exp", "log": False},
}
...
def optuna_hp_tuning(X: np.ndarray, y: np.ndarray, n_trials: int = 100,
                     timeout: int = 600, seed: int = 42) -> tuple:
    """Optuna TPE HP tuning with MedianPruner. Inner 5-fold CV.

    Returns:
        best_params: dict of best hyperparameters
        trial_history: list of (trial_number, value, best_so_far) for convergence plots
    """
...
def train_property(prop_name: str, prop_config: dict, desc_df: pd.DataFrame,
    ...
    """Full nested-CV pipeline for a single property."""
    t0_total = time.time()
    ...
    # --- Setup directories ---
    # Use prop_name directly for constraints, "40C" subdir for thermo
    is_constraint = prop_name in ("fp", "bp", "mp", "dc")
    if is_constraint:
    ...
    fold_best_params = []
    # (test_idx, shap_values, X_test_scaled, selected_feature_names)
    fold_shap_data = []
    ...
    summary = {
        "property": prop_name,
        "temperature": "40C" if prop_name not in ("fp", "bp", "mp", "dc") else "N/A",
        "r2": round(r2_cv, 4),
...
def main():
    parser = argparse.ArgumentParser(
        description="Train CHNO XGBoost models (Mordred + Optuna)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--properties", nargs="+", default=["all"],
        help="Properties to train (density viscosity tc cpsat beta fom1 fp bp mp dc) or 'all'",
    )
    ...
    # Train
    logger.info("=" * 70)
    logger.info("Training %d properties (%d Optuna trials/fold, %ds timeout)",
    ...
    for prop_name in properties:
        ...
        if summary:
            key = f"{prop_name}_40C" if prop_name not in ("fp", "bp", "mp", "dc") else prop_name
            results[key] = summary
    ...
    # Summary table
    print(f"\n{'=' * 80}")
    print("CHNO MODELS (5-fold nested CV, Mordred+RDKit, Optuna)")
    print(f"{'=' * 80}")
