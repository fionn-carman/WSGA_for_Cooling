...
the CHNO property + constraint datasets.

Produces: training/data/CHNO/descriptors_rdkit_mordred.csv
...
# Thermophysical property CSVs
THERMO_CSVS = [
    "density_chno_cleaned.csv",
    "viscosity_chno_cleaned.csv",
    "tc_chno_cleaned.csv",
    "cpsat_chno_cleaned.csv",
    "beta_chno_cleaned.csv",
    "fom1_chno_cleaned.csv",
]

# Constraint CSVs (in constraints/ subdirectory)
CONSTRAINT_CSVS = [
    "constraints/flashpoint_cleaned.csv",
    "constraints/BP-Measured_cleaned.csv",
    "constraints/MP-Measured_cleaned.csv",
    "constraints/DC_exp_cleaned.csv",
]
...
def collect_unique_smiles() -> list:
    """Gather unique SMILES from all property and constraint CSVs."""
    all_smiles = set()
    for csv_name in THERMO_CSVS + CONSTRAINT_CSVS:
