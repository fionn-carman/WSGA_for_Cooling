#!/usr/bin/env python3
"""
XGBoost on Morgan fingerprints only for FOM1 prediction.

Features: 2048-dim Morgan fingerprints (no descriptors).
...
def main():
    parser = argparse.ArgumentParser(
        description="Train XGBoost on Morgan fingerprints for FOM1 prediction",
    ...
    logger.info("Feature matrix: %s (fingerprints only)", X.shape)
    ...
    for fold_id in folds:
        logger.info("=" * 50)
        logger.info("FOLD %d — XGBoost Fingerprints", fold_id)
        logger.info("=" * 50)
        t0 = time.time()
        ...
        pred_df.to_csv(data_dir / f"xgboost_fingerprints_fold{fold_id}_predictions.csv", index=False)

        # Save metrics
        metrics = {
            "model": "xgboost_fingerprints",
            "target": args.target,
            "fold": fold_id,
            "r2": float(r2),
            "rmse": float(rmse),
        ...
        with open(data_dir / f"xgboost_fingerprints_fold{fold_id}_metrics.json", "w") as f:
            json.dump(metrics, f, indent=2)

        # Save model
        joblib.dump({"model": best_model, "scaler": scaler, "params": search.best_params_},
                     data_dir / f"xgboost_fingerprints_fold{fold_id}_model.joblib")

    logger.info("XGBoost Fingerprints training complete.")
