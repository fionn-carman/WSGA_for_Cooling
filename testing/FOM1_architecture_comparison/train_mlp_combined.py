#!/usr/bin/env python3
"""
MLP on combined descriptors + fingerprints for FOM1 prediction.
Larger first layer (512) to handle higher input dimensionality.
"""
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims=(512, 256, 128, 64), dropout=0.1):
        super().__init__()
        layers = []
        prev_dim = input_dim
...
def main():
    parser = argparse.ArgumentParser(
        description="Train MLP on combined descriptors+fingerprints for FOM1",
    ...
    X_desc = df[descriptor_columns].values.astype(np.float32)
    X_fp = np.load(data_dir / "fingerprints.npy")
    X = np.hstack([X_desc, X_fp])
    y = df[args.target].values.astype(np.float32)
    logger.info("Target: %s", args.target)
    logger.info("Combined features: %d (desc=%d + fp=%d)", X.shape[1], X_desc.shape[1], X_fp.shape[1])

    folds = range(5) if args.fold is None else [args.fold]

    for fold_id in folds:
        logger.info("=" * 50)
        logger.info("FOLD %d — MLP Combined", fold_id)
        logger.info("=" * 50)
        t0 = time.time()
        ...
        pd.DataFrame({"SMILES": df.iloc[test_idx]["SMILES"].values, "y_true": y_test, "y_pred": y_pred})\
            .to_csv(data_dir / f"mlp_combined_fold{fold_id}_predictions.csv", index=False)

        with open(data_dir / f"mlp_combined_fold{fold_id}_metrics.json", "w") as f:
            json.dump({"model": "mlp_combined", "target": args.target, "fold": fold_id, "r2": float(r2),
        ...
        torch.save({"state_dict": best_state, "input_dim": X_train.shape[1]},
                    data_dir / f"mlp_combined_fold{fold_id}_model.pt")

        with open(data_dir / f"mlp_combined_fold{fold_id}_history.json", "w") as f:
            json.dump(history, f)

    logger.info("MLP Combined training complete.")
