...
# ============================================================
# REINVENT Production FP × MolPrice Sweep — NO SPHERE FILTER
# ============================================================
#
# Same grid as reinvent_fp_molprice_production.sh but WITHOUT
# the sphere diversity filter. Sphere filter prevents reliable
# convergence to the global optimum at fp100 (only 3/5 seeds).
#
...
#
# No diversity filter — replay buffer only for exploration.
#
# Usage:
#   qsub reinvent_fp_molprice_production_nosphere.sh
#   bash reinvent_fp_molprice_production_nosphere.sh <array_index>
# ============================================================
...
output_dir="../outputs/reinvent_fp_molprice_nosphere/${FP_LABEL}_${LEVEL}_seed${SEED}"
mkdir -p "$output_dir"

log_file="${output_dir}/config.log"
{
    echo "========================================"
    echo "REINVENT Production FP × MolPrice (No Sphere)"
    echo "========================================"
    echo "Array index:       $N"
    ...
    echo "Seed:              $SEED"
    echo "Sigma:             $SIGMA"
    echo "Diversity:         none (replay buffer only)"
    echo "Biodeg:            on"
