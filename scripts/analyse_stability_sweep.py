"""
Analyse stability × MolPrice sweep results (4 categories).

Parses output directories from stability_molprice_sweep.sh, which sweeps
...
Figures generated:
  1. 2×2 Pareto front panels (cost vs FOM1) — one per category
  2. FOM1 vs MolPrice threshold level (4 category lines)
  3. MolPrice distribution by threshold (4 panels)
  4. Convergence curves (2×2 grid)

Usage:
    python analyse_stability_sweep.py --sweep_dir ../outputs/stability_molprice_sweep
    python analyse_stability_sweep.py --sweep_dir ../outputs/stability_molprice_sweep \
        --baseline_csv ../results/fom1_dataset_categories.csv
"""
...
def plot_pareto_2x2(runs_df, sweep_dir, out_dir, baseline_df=None):
    ...
    for category, (row, col) in panel_map.items():
        ...
        wsga = wsga.sort_values("FOM1_avg", ascending=False)
...
def plot_fom1_vs_threshold(runs_df, out_dir):
    ...
    for category in CATEGORY_ORDER:
        ...
        agg = sub.groupby("level")["best_FOM1_avg"].agg(["mean", "std", "count"])
...
def plot_convergence_2x2(runs_df, sweep_dir, out_dir):
    ...
    for ax in axes[:, 0]:
        ax.set_ylabel("Best Valid FOM1 (avg)", fontsize=11)
...
def analyse_stability_sweep(sweep_dir, top_n_molecules=50,
    ...
    print(f"{'Level':<12} {'Category':<20} {'Best FOM1_avg':>18}   "
    ...
    for category in CATEGORY_ORDER:
        for level in LEVEL_ORDER:
            ...
            if sub.empty:
                continue
            fom1_mean = sub["best_FOM1_avg"].mean()
            fom1_std = sub["best_FOM1_avg"].std()
    ...
    if all_valid_mols:
        ...
        header = (f"{'Rank':<5} {'SMILES':<42} {'FOM1_avg':>10} "
    ...
    if all_valid_mols:
        for category in CATEGORY_ORDER:
            ...
            cat_unique = cat_unique.dropna(subset=["MolPrice", "FOM1_avg"])
            if cat_unique.empty:
                continue

            afford = -cat_unique["MolPrice"].values
            fom1 = cat_unique["FOM1_avg"].values
            fronts = _pareto_fronts(afford, fom1, n_fronts=1)
            ...
            if fronts and len(fronts[0]) > 0:
                ...
                save_cols = [c for c in [smiles_col, "FOM1_40", "FOM1_100",
                                         "FOM1_avg", "MolPrice",
