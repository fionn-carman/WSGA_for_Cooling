...
# ============================================================
# Niching Sweep — Radius x Tau Calibrated Study (FP >= 125 C)
# ============================================================
#
# Identical to niching_sweep.sh but with FP threshold = 398 K
# (125 C) to test whether niching matters more when the
# feasible region is smaller and landscape harder.
#
# Conditions (16):
...
#   FP = 398 K (125 C), BP = 70 C, MP = -30 C, DC = 8, SC = 3, Tox = 3
#
# Total: 16 * 5 = 80 jobs
#
# Usage:
#   qsub niching_sweep_fp125.sh
#   qsub -J 0-0 niching_sweep_fp125.sh       # single test job
#   bash niching_sweep_fp125.sh <array_index> # local test
# ============================================================
...
if [ -z "$N" ]; then
    if [ $# -eq 1 ]; then
        ...
        echo "Usage: bash niching_sweep_fp125.sh <array_index>"
        exit 1
    fi
fi
...
if [ "$RADIUS" -eq 0 ]; then
    ...
    RADIUS_FLAG=""
    output_dir="../outputs/niching_sweep_fp125/no_niching_seed${SEED}"
else
    NICHING_FLAG=""
    RADIUS_FLAG="--niching_radius $RADIUS"
    output_dir="../outputs/niching_sweep_fp125/r${RADIUS}_tau${TAU}_seed${SEED}"
fi
...
echo "========================================" > "$log_file"
echo "WSGA Niching Sweep (FP >= 125 C)" >> "$log_file"
...
echo "FP threshold:      398 K (125 C)" >> "$log_file"
